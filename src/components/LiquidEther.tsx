...
interface LiquidEtherWebGL {
  ...
  resize: () => void; start: () => void; pause: () => void; dispose: () => void;
}
...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    class CommonClass {
      width=0; height=0; aspect=1; pixelRatio=1; isMobile=false; breakpoint=768;
      fboWidth:number|null=null; fboHeight:number|null=null; time=0; delta=0;
      container:HTMLElement|null=null; renderer:THREE.WebGLRenderer|null=null;
      clock:THREE.Clock|null=null;
      init(container:HTMLElement) {
        this.container=container; this.pixelRatio=Math.min(window.devicePixelRatio||1,2);
        this.resize();
    ...
    class MouseClass {
      ...
      private _onMM=this.onDocMM.bind(this); private _onTS=this.onDocTS.bind(this);
      private _onTM=this.onDocTM.bind(this); private _onTE=this.onTE.bind(this);
      private _onDL=this.onDL.bind(this);
      init(container:HTMLElement) {
        ...
        this.listenerTarget.addEventListener('mousemove',this._onMM);
        this.listenerTarget.addEventListener('touchstart',this._onTS,{passive:true});
        this.listenerTarget.addEventListener('touchmove',this._onTM,{passive:true});
        this.listenerTarget.addEventListener('touchend',this._onTE);
        this.docTarget?.addEventListener('mouseleave',this._onDL);
      }
      dispose() {
        if(this.listenerTarget){this.listenerTarget.removeEventListener('mousemove',this._onMM);
        ...
        if(this.docTarget) this.docTarget.removeEventListener('mouseleave',this._onDL);
        this.listenerTarget=null; this.docTarget=null; this.container=null;
      }
      private isPI(cx:number,cy:number){if(!this.container)return false;const r=this.container.getBoundingClientRect();return cx>=r.left&&cx<=r.right&&cy>=r.top&&cy<=r.bottom;}
      private uHS(cx:number,cy:number){this.isHoverInside=this.isPI(cx,cy);return this.isHoverInside;}
      ...
      onDocMM(event:MouseEvent){
        if(!this.uHS(event.clientX,event.clientY))return;
      ...
      onDocTS(event:TouchEvent){if(event.touches.length!==1)return;const t=event.touches[0];if(!this.uHS(t.clientX,t.clientY))return;if(this.onInteract)this.onInteract();this.setCoords(t.clientX,t.clientY);this.hasUserControl=true;}
      onDocTM(event:TouchEvent){if(event.touches.length!==1)return;const t=event.touches[0];if(!this.uHS(t.clientX,t.clientY))return;if(this.onInteract)this.onInteract();this.setCoords(t.clientX,t.clientY);}
      onTE(){this.isHoverInside=false;} onDL(){this.isHoverInside=false;}
      update(){
    ...
    class AutoDriver {
      ...
      update(){
        ...
        if(!this.active)return; this.mouse.isAutoActive=true;
    ...
    class ShaderPass {
      props:any; uniforms?:Uniforms; scene:THREE.Scene|null=null; camera:THREE.Camera|null=null;
    ...
    class Advection extends ShaderPass {
      ...
      createBoundary(){const g=new THREE.BufferGeometry();const v=new Float32Array([-1,-1,0,-1,1,0,-1,1,0,1,1,0,1,1,0,1,-1,0,1,-1,0,-1,-1,0]);g.setAttribute('position',new THREE.BufferAttribute(v,3));const m=new THREE.RawShaderMaterial({vertexShader:line_vert,fragmentShader:advection_frag,uniforms:this.uniforms!});this.line=new THREE.LineSegments(g,m);this.scene!.add(this.line);}
      update(args:any){const{dt,isBounce,BFECC}=args;if(!this.uniforms)return;this.uniforms.dt.value=dt;this.line.visible=isBounce;this.uniforms.isBFECC.value=BFECC;super.update();}
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(args:any){const{mouse_force,cellScale,cursor_size}=args;const u=(this.mouse.material as THREE.RawShaderMaterial).uniforms;u.force.value.set((Mouse.diff.x/2)*mouse_force,(Mouse.diff.y/2)*mouse_force);u.center.value.set(Mouse.coords.x,Mouse.coords.y);u.scale.value.set(cursor_size,cursor_size);super.update();}
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update(args:any){const{iterations}=args;for(let i=0;i<iterations;i++){const pi=i%2===0?this.props.output0:this.props.output1;const po=i%2===0?this.props.output1:this.props.output0;this.uniforms!.pressure.value=pi.texture;this.props.output=po;super.update();}return iterations%2===0?this.props.output0:this.props.output1;}
    }
    ...
    class Simulation {
      ...
      createAllFBO(){const type=this.getFloatType();const opts={type,depthBuffer:false,stencilBuffer:false,minFilter:THREE.LinearFilter,magFilter:THREE.LinearFilter,wrapS:THREE.ClampToEdgeWrapping,wrapT:THREE.ClampToEdgeWrapping};for(const key in this.fbos)this.fbos[key]=new THREE.WebGLRenderTarget(this.fboSize.x,this.fboSize.y,opts);}
      createShaderPass(){
    ...
    const io=new IntersectionObserver(entries=>{const isVisible=entries[0].isIntersecting;isVisibleRef.current=isVisible;if(webglRef.current)isVisible&&!document.hidden?webglRef.current.start():webglRef.current.pause();},{threshold:0.1});
    ...
    return ()=>{if(rafRef.current)cancelAnimationFrame(rafRef.current);ro.disconnect();io.disconnect();webglRef.current?.dispose();webglRef.current=null;};
  },[colors,autoDemo,autoSpeed,autoIntensity,takeoverDuration,autoResumeDelay,autoRampDuration]);

  return (
    <div ref={mountRef} className={className} style={{width:'100%',height:'100%',position:'relative',overflow:'hidden',...style}} />
  );
}
